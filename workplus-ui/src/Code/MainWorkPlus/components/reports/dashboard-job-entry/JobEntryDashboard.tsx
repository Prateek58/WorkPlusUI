...
interface JobEntryDashboardProps {
  embedded?: boolean; // When true, don't wrap in DashboardLayout and don't show back button
}

const JobEntryDashboard: React.FC<JobEntryDashboardProps> = ({ embedded = false }) => {
    ...
    <Container maxWidth={embedded ? false : "xl"} sx={{ mt: embedded ? 0 : 9, mb: 4, px: embedded ? 0 : 3 }}>
      ...
      {!embedded && (
        <Box sx={{ display: 'flex', alignItems: 'center', mb: 3 }}>
          ...
          {/* Action buttons - commented out for now, can be re-enabled later
